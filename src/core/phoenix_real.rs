...
/// Real Phoenix DEX API client with on-chain data parsing
pub struct PhoenixRealClient {
    rpc_client: RpcClient,
    program_id: Pubkey,
    commitment: CommitmentConfig,
}

impl PhoenixRealClient {
    pub fn new(rpc_url: String, commitment: CommitmentConfig) -> Result<Self, PhoenixError> {
        let rpc_client = RpcClient::new_with_commitment(rpc_url, commitment);
        let program_id = Pubkey::from_str("PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY")
    ...
    pub async fn fetch_real_phoenix_markets(&self) -> Result<Vec<PhoenixMarket>, PhoenixError> {
    ...
    async fn get_known_markets_as_phoenix_markets(&self) -> Result<Vec<PhoenixMarket>, PhoenixError> {
        ...
        for (address, (name, base_mint, quote_mint)) in known_markets {
            ...
            if self.real_market_exists(&address).await {
                let market = PhoenixMarket {
                    address,
                    name,
                    base_mint,
    ...
    pub async fn get_real_market_details(&self, market_address: &str) -> Result<PhoenixMarket, PhoenixError> {
    ...
    /// Fetch real orderbook data from on-chain market account
    pub async fn fetch_real_phoenix_orderbook(
        &self, 
        market_address: &str, 
        depth: Option<u32>
    ) -> Result<PhoenixOrderbook, PhoenixError> {
        let market_pubkey = Pubkey::from_str(market_address)
            .map_err(|e| PhoenixError::Parse(format!("Invalid market address: {}", e)))?;
        ...
        let market = self.get_real_market_details(market_address).await?;
    ...
    pub async fn fetch_multiple_real_orderbooks(
        &self, 
        market_addresses: &[&str], 
        depth: Option<u32>
    ) -> Result<HashMap<String, PhoenixOrderbook>, PhoenixError> {
        let mut results = HashMap::new();
        
        // For simplicity, fetch sequentially for now
        for &market_address in market_addresses {
            match self.fetch_real_phoenix_orderbook(market_address, depth).await {
                Ok(orderbook) => {
    ...
    pub async fn real_market_exists(&self, market_address: &str) -> bool {
